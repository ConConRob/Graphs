...
def move_toward_dead_end(player, map):
    ...
    for key in map_piece:
        if map_piece[key] == '?':
            ...
            move_toward_dead_end(player, map)
            break
   
    
    

    # if no way to move in this room you are at the dead end
    # stop recurisive loop

traversalPath = []
rooms_visited = { player.currentRoom.id: room_direction(player.currentRoom) }

move_toward_dead_end(player, rooms_visited)
print(rooms_visited)
# while len(rooms_visited) < len(roomGraph):
    # go to a dead end (room with no neighbour unexplored rooms)
    

    # if not player.currentRoom.id in rooms_visited:
    #     # add it to rooms visited
        
    #     pass
       
# find closest unvisted room
# go to this room
# repeat
...
# q = Queue()
# # queue the room with the path => last room in path is the room that needs to be looked at
# q.enqueue([start])
# visited = []
# path_of_rooms = []
# while q.size() > 0:
...
#         # put room as visited
#         visited.append(room)
#         # check all directions to go
#         exits = room.getExits()   
#         # get the room in each direction
#         for direction in exits:
#             neighbour = room.getRoomInDirection(direction)
#             print(neighbour)
#             if neighbour == '?':
#                 # found the exit
#                 print('found exit')
#             if not neighbour in visited:
#                 print(f'=========================new room to queue=======')
